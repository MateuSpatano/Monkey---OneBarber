...
export default function Users() {
  ...
  const { isAdmin: currentUserIsAdmin } = usePermissionsContext();

  const filteredUsers = users.filter((user) => {
    // Hide admin users from non-admin users (barbers, owners, etc.)
    if (!currentUserIsAdmin && user.role_name === 'Administrador') return false;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        <div className="flex flex-col gap-2">
          <h1 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight underline decoration-primary/20 underline-offset-8">Usuários</h1>
          <p className="text-muted-foreground font-medium text-sm sm:text-base">Gerencie os usuários e permissões do sistema</p>
        </div>
        {canCreate('users') && (
          <Button onClick={handleCreate} className="premium-button-solid h-11 px-6 shadow-xl">
      ...
      <Card className="premium-card border-none shadow-2xl rounded-[32px] overflow-hidden">
        <CardHeader className="pb-6 p-8 bg-zinc-50/50 border-b border-black/5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
            ...
            <Button variant="ghost" onClick={clearFilters} className="premium-button-ghost border-none h-10 px-4">
              Limpar Filtros
            </Button>
          </div>
        </CardHeader>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl hover:bg-zinc-100 transition-all">
