...
export default function ReportGroups() {
  ...
  const handleExport = (group: ReportGroup, format: 'xml' | 'pdf') => {
    if (format === 'xml') {
...
<report_group>
  ...
  <is_active>${group.is_active}</is_active>
  <created_at>${group.created_at}</created_at>
      ...
      a.href = url;
      a.download = `relatorio-${group.name.toLowerCase().replace(/\s+/g, '-')}.xml`;
      a.click();
      URL.revokeObjectURL(url);
      toast({ title: 'XML exportado com sucesso' });
    } else {
      // Generate simple PDF using a printable HTML window
