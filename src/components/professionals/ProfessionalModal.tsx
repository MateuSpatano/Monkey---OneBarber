...
import { User, Paperclip, Percent, Loader2 } from 'lucide-react';
...
export function ProfessionalModal({ open, onOpenChange, mode, professional, onSuccess }: ProfessionalModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col rounded-[32px] border-none shadow-2xl p-6 sm:p-8">
        <DialogHeader className="mb-6">
          <DialogTitle className="text-2xl font-black tracking-tight">{title}</DialogTitle>
        </DialogHeader>

        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col">
          <TabsList className="grid w-full grid-cols-3 bg-secondary/50 p-1.5 rounded-2xl h-14">
            <TabsTrigger value="personal" className="flex items-center gap-2 rounded-xl data-[state=active]:bg-white data-[state=active]:shadow-sm font-bold text-xs uppercase tracking-wider">
              <User className="h-4 w-4" />
              Cadastro
            </TabsTrigger>
            <TabsTrigger value="attachments" className="flex items-center gap-2 rounded-xl data-[state=active]:bg-white data-[state=active]:shadow-sm font-bold text-xs uppercase tracking-wider">
              <Paperclip className="h-4 w-4" />
              Anexos
            </TabsTrigger>
            <TabsTrigger value="commissions" className="flex items-center gap-2 rounded-xl data-[state=active]:bg-white data-[state=active]:shadow-sm font-bold text-xs uppercase tracking-wider">
        ...
        <div className="flex justify-end gap-3 pt-6 border-t border-black/5">
          ...
          {!isReadOnly && (
            <Button onClick={handleSubmit} disabled={loading} className="premium-button-solid h-11 px-8 shadow-lg">
              {loading ? (
                <>
